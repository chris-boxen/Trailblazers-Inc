 ...
 * TEC single event view override for athletic-meet events.
 *
 * IMPORTANT — TEC rendering pipeline:
 *   1. TEC intercepts `template_include` and loads views/v2/default-template.php.
 *   2. default-template.php calls get_header(), renders the view, then get_footer().
 *   3. The view is this file: views/single-event.php (theme override: tribe/events/single-event.php).
 *
 * This file is loaded as a VIEW (inner content only). Do NOT call get_header(),
 * get_footer(), or use the WordPress while(have_posts()) loop. The header and footer
 * are already handled by default-template.php. Use get_queried_object_id() for the
 * current post ID.
 *
 * A category guard restricts the full meet layout to athletic-meet events.
 * All other tribe_events categories (practice, team-event, community-run) render
 * a minimal title fallback.
 *
 * Sections:
 ...
 *
 * TEC postmeta (not ACF):
 *   _EventStartDate     — meet start datetime (Y-m-d H:i:s)
 *   _EventVenueID       — linked tribe_venue post ID
 *   _VenueCity          — on the venue post
 *   _VenueStateProvince — on the venue post
 *
 * FIELD NAME NOTES:
 ...
 *   - results_status is a top-level ACF field on tribe_events (no group wrapper).
 *
...
// -------------------------------------------------------------------------
// CATEGORY GUARD
// This view fires for all tribe_events. Only render the full meet layout
...
<div class="tb-single tb-tec-event">
	<section class="tb-single-header">
		<div class="tb-single-headline">
			<h1 class="tb-single-title"><?php echo esc_html( get_the_title( $meet_id ) ); ?></h1>
		</div>
	</section>
</div>
<?php
