...
### Built physical upload form and submission hook

**Form 16 — Submit Physical** (permanent, `requireLogin`)

Fields:
- Field 1: Family Post ID (hidden, `gform_field_value_tb_family_post_id`)
- Field 2: Season ID (hidden, `gform_field_value_tb_active_season_id`)
- Field 3: Athlete (select, enrollment-scoped via `gform_pre_render`)
- Field 4: Physical Form (file upload, media library off, 10MB, pdf/jpg/png/doc/docx)
- Field 5: Email (hidden, `gform_field_value_tb_user_email`)
- Field 6: Athlete Name (hidden, populated via `gform_pre_submission`)

Key decisions:
- Athlete dropdown scoped to active-season enrollments for logged-in family,
  not all Active athletes. Implemented via `gform_pre_render` rather than GPPA
  because GPPA cannot cross-join post types (athlete + enrollment + season).
- Form is permanent — season injected at submission time via filter.
  `reg_physicals_form_id` on options page will not need to change year to year.
- Files stored outside media library (`uploadMediaLibrary: false`) — URLs
  stored in `file_url` text field on Athletic Physical post.
- One Physical post per athlete per season — re-submission updates the
  existing post rather than creating a duplicate. Revision history preserved.

`tb_handle_physicals` in `inc/gravity-helpers.php`:
- Resolves family, season, athlete from entry fields
- Checks for existing Physical post (athlete + season) — updates if found,
  creates if not
- Sets: `athlete`, `family`, `season_received`, `submission_method`,
  `digital_copy_on_file`, `file_url`
- Updates Enrollment `physical_status` to Received for matching athlete + season

`[tb_reg_form type="physicals"]` in `inc/registration-helpers.php`:
- Bypasses registration open/close date gate entirely
- Physicals submission window is independent of registration status

ACF changes:
- `group_tb_athletic_physical` — replaced `link` (ACF link type) with
  `file_url` (text). ACF JSON synced.
- `post_type_tb_athletic_physical` — `publicly_queryable` set to false.

WP pages:
- `/registration/physicals/` — `[tb_reg_form type="physicals"]`
- `/registration/physicals/confirmation/` — `[tb_reg_confirmation type="physicals"]`
